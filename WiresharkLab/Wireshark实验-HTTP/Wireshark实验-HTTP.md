...
17. 浏览器从两个网站串行还是并行下载了两张图片？请说明(使用的是顺序请求，因为两个GET请求发出去的时间是有时间差的)。
...
![HTTP认证图](http密码输入后的get报文.PNG)
- 这个图中的用户名和密码是Credentials（证书（注意：新版wireshark自带解码功能）: eth-students（用户名）networks（密码）
